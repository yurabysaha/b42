...
class Message(BaseMethod):
    def __init__(self, view):
        ...
        candidate_for_message = [i for i in self.view.candidate_list if
                                 i['send_connect'] and i['accept_connect'] and not i['send_message']]
    ...
    def send_message(self, user):
        ...
        button.click()
        time.sleep(0.5)
        message = self.view.data['connect_message_text']
        ...
        time.sleep(0.5)
        self.chrome.find_element_by_xpath("//button[contains(@data-control-name, 'overlay.close')]").click()
        self.view.process_log("Message sent to - %s" % user['candidate']['full_name'])
        return True
    ...
    def update_on_view(self, linkedin_url):
        ...
        if line.winfo_children()[5].cget('text') == 'M':
            ...
            if resp.status_code == 200:
                data = resp.json()
                line.winfo_children()[5].config(bg='green', fg='white')
